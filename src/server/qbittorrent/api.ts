...
export class QBittorrentApi {
  ...
  async torrentHashes() {
    const data = await this.client.getAllData();
    return new Set(data.torrents.map((torrent) => torrent.id));
  }
  ...
  async download(episode: Episode, knownTorrentHashes?: ReadonlySet<string>) {
    const exists = knownTorrentHashes?.has(episode.torrent) ?? (await this.hasTorrent(episode.torrent));
  ...
  private async hasTorrent(hash: string) {
    try {
      await this.client.getTorrent(hash);
      return true;
    } catch (error) {
      if ((error as Error).message === 'Torrent not found') return false;
      throw error;
    }
  }
